...
async def on_message(message):
    ...
    if message.content.startswith("<@793941611418615829>"):
        await message.channel.trigger_typing()
        time.sleep(1)
        if "combien" in message.content and "membres" in message.content:
          print("TRUE")
          guild = bot.get_guild("793946468570103828")
          print(guild.member_count)
          message.channel.send(guild.member_count)
...
async def dex(ctx, pokémon):
	msg = await ctx.respond(embed=discord.Embed(
	    title="Veuillez Patienter :clock:",
	    description=
	    "Votre commande est en cours de traitement, cela peut prendre jusqu'à 5 secondes."
	))
	#msg=msg.id
	#channel = ctx.channel
	...
	try:
		...
		if infos['type1'] == infos['type2']:
			type_ = type_to_emote[french_types[infos['type1'].replace(
			    "TYPE_", '').lower()]]
		else:
			type_ = type_to_emote[french_types[infos['type1'].replace(
			    "TYPE_", '').lower()]] + ' ' + type_to_emote[french_types[
			        infos['type2'].replace("TYPE_", '').lower()]]
		print(infos['ability'])
		ability = [
		    get_ability(talent.replace(" ", "")) for talent in
		    infos["ability"].replace("{", "").replace("}", "").split(",")
		]
		if len(ability) != 3:
			ability.append("-------")
		print(ability)
		...
		for i in range(len(dico)):
			embed_content += list(dico.keys())[i] + ' : ' + list(
			    dico.values())[i] + '\n'
		sumbs = sum(
		    int(e) for e in [
		        infos['hp'], infos['atk'], infos['def'], infos['sp.atk'],
		        infos['sp.def'], infos['speed']
		    ])
		embed = discord.Embed(
		    title=f"{pokemon[1]}",
		    description=
		    f"__N°{list(loca.keys()).index(pokemon[1])+1}__ (Nom 🇬🇧: {pokemon[0]})\n__**Type(s)**__ : {type_}\n__**Talents:**__ {ability}\n __**Stats de base**__ :\nPV : {infos['hp']}\nAtq : {infos['atk']}\nDef : {infos['def']}\nAtq.Spé : {infos['sp.atk']}\nDef.Spé : {infos['sp.def']}\nVit : {infos['speed']}\nBS : {sumbs}\n__**Learnset**__\n{embed_content}\n__**Localisation**__ : {loca[pokemon[1]]}",
		    color=type_to_color[french_types[infos['type1'].replace(
		        "TYPE_", '').lower()]])
		print(
		    f'https://raw.githubusercontent.com/Arlequiin/pokeemerald-expansion/master/graphics/pokemon/{pokemon[0].lower()}{pokemon[2]}/front.png'
		)
		file = discord.File(
		    removebg(
		        f'https://raw.githubusercontent.com/Arlequiin/pokeemerald-expansion/master/graphics/pokemon/{pokemon[0].lower()}{pokemon[2]}/front.png'
	...
	#-------------------------------------------
		...
		print(e)
		await msg.edit_original_response(embed=discord.Embed(
		    title="❌ ERREUR",
		    description=
		    "Vous avez :\n- Mal saisi le nom du Pokémon (ex : `Majspic` au lieu de `Majaspic`)\n - Vous avez saisi le nom d'un Pokémon de la 9ème génération (ex : `Poussacha`)\n*Si rien de tout cela n'est vrai, veuillez contacter `Arlequiin#1853`*"
		))
